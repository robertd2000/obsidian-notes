...
**EXPLAIN** — это команда SQL, которая показывает план выполнения запроса, не выполняя его фактически. Это основной инструмент для оптимизации производительности запросов.
...
-- Результат (PostgreSQL пример):
```
```
QUERY PLAN
---------------------------------------------------------------
Index Scan using idx_users_email on users  (cost=0.15..8.17 rows=1 width=124)
...
QUERY PLAN
----------------------------------------------------------------------------------------------------------------
Index Scan using idx_users_email on users  (cost=0.15..8.17 rows=1 width=124) (actual time=0.025..0.026 rows=1 loops=1)
  Index Cond: (email = 'test@example.com'::text)
...
```
Seq Scan on orders  (cost=0.00..1550.00 rows=50000 width=40)
```
- **Seq Scan** - тип операции (последовательное сканирование)
- **cost=0.00..1550.00** - стоимость (начальная..общая)
- **rows=50000** - ожидаемое количество строк
- **width=40** - средний размер строки в байтах
...
- **Второе число (1550.00)** - стоимость получения всех строк
- Измеряется в условных единицах (обычно 1 = чтение одной страницы с диска)

## 🏗️ Типы операций в плане выполнения
...
```
```
Seq Scan on large_table  (cost=0.00..12500.00 rows=100000 width=64)
  Filter: (status = 'active'::text)
```
**Что означает:** Полное сканирование таблицы. Используется когда:
- Нет подходящего индекса
- Большой процент строк удовлетворяет условию
- Таблица маленькая
...
```
```
Index Scan using users_pkey on users  (cost=0.15..8.17 rows=1 width=124)
  Index Cond: (id = 1000)
```
**Что означает:** Поиск по индексу с последующим чтением данных из таблицы.

### **3. Index Only Scan**
```sql
EXPLAIN SELECT id FROM users WHERE id BETWEEN 100 AND 200;
```
```
Index Only Scan using users_pkey on users  (cost=0.15..12.42 rows=101 width=4)
...
```
```
Bitmap Heap Scan on users  (cost=12.34..45.67 rows=100 width=64)
  Recheck Cond: ((age > 30) AND (city = 'New York'::text))
  ->  BitmapAnd  (cost=12.34..12.34 rows=100 width=0)
        ->  Bitmap Index Scan on idx_users_age  (cost=0.00..6.12 rows=500 width=0)
              Index Cond: (age > 30)
        ->  Bitmap Index Scan on idx_users_city  (cost=0.00..6.12 rows=500 width=0)
              Index Cond: (city = 'New York'::text)
```
**Что означает:** Эффективный поиск по нескольким индексам с объединением результатов.
...
```
```
Nested Loop  (cost=0.15..1250.45 rows=10000 width=64)
  ->  Seq Scan on users u  (cost=0.00..125.00 rows=1000 width=36)
  ->  Index Scan using idx_orders_user_id on orders o  (cost=0.15..1.12 rows=10 width=28)
        Index Cond: (user_id = u.id)
```
**Что означает:** Для каждой строки из внешней таблицы ищутся соответствия во внутренней.
...
```
```
Hash Join  (cost=150.00..1250.00 rows=10000 width=64)
  Hash Cond: (o.user_id = u.id)
  ->  Seq Scan on orders o  (cost=0.00..750.00 rows=50000 width=28)
  ->  Hash  (cost=125.00..125.00 rows=1000 width=36)
        ->  Seq Scan on users u  (cost=0.00..125.00 rows=1000 width=36)
```
**Что означает:** Создается хэш-таблица для одной из таблиц, затем происходит соединение.
...
```
```
Merge Join  (cost=1250.67..2450.89 rows=50000 width=128)
  Merge Cond: (t1.id = t2.id)
  ->  Index Scan using table1_pkey on table1 t1  (cost=0.15..1250.45 rows=50000 width=64)
  ->  Index Scan using table2_pkey on table2 t2  (cost=0.15..1250.45 rows=50000 width=64)
```
**Что означает:** Обе таблицы сканируются в отсортированном порядке и объединяются.

## 📊 Анализ реальных примеров

### **Пример 1: Плохой запрос - полное сканирование**
```sql
EXPLAIN ANALYZE 
SELECT * FROM orders 
...
```
```
Seq Scan on orders  (cost=0.00..15500.00 rows=50000 width=64) (actual time=0.125..450.250 rows=50000 loops=1)
  Filter: (EXTRACT(year FROM order_date) = 2024)
  Rows Removed by Filter: 500000
Planning Time: 0.150 ms
Execution Time: 455.300 ms
```

**Проблемы:**
- Полное сканирование таблицы (Seq Scan)
- Фильтр применяется к каждой строке
- Обработка 550,000 строк для получения 50,000

**Решение:**
```sql
-- Создаем индекс для функционального выражения
CREATE INDEX idx_orders_year ON orders (EXTRACT(YEAR FROM order_date));
...
### **Пример 2: Эффективный запрос с индексом**
```sql
EXPLAIN ANALYZE 
SELECT * FROM users 
...
**Анализ:**
- Быстрый поиск по индексу (0.047 ms)
- Фильтр применяется после поиска по индексу
- Идеальная производительность

### **Пример 3: JOIN с разными типами соединений**
```sql
EXPLAIN ANALYZE
...
-- Показать распределение по рабочим процессам (для параллельных запросов)
EXPLAIN (ANALYZE, SUMMARY) SELECT * FROM large_table;
```
...
- **Rows Removed by Filter** - строки отфильтрованные
- **Buffers** - использование кэша
- **Loops** - количество итераций

### **Анализ использования памяти**
```
Buffers: shared hit=125 read=250
```
- **shared hit** - чтение из кэша
- **shared read** - чтение с диска
- **Цель**: максимизировать hit, минимизировать read
...
```
```
Seq Scan on logs  (cost=0.00..12500.00 rows=1000 width=256) (actual time=5.250..125.500 rows=1000 loops=1)
...
### **Проблема 2: Неселективные индексы**
```sql
EXPLAIN ANALYZE SELECT * FROM users WHERE status = 'active';
```
```
Index Scan using idx_users_status on users  (cost=0.15..8500.00 rows=80000 width=124) (actual time=0.125..125.250 rows=80000 loops=1)
  Index Cond: (status = 'active'::text)
```
**Решение:** Индекс не помогает когда выбирается большая часть таблицы.

### **Проблема 3: Функции в условиях WHERE**
```sql
EXPLAIN ANALYZE SELECT * FROM users WHERE LOWER(name) = 'john';
```
```
Seq Scan on users  (cost=0.00..12500.00 rows=1000 width=124) (actual time=2.250..125.500 rows=1 loops=1)
...
```sql
-- Поиск самых дорогих запросов (PostgreSQL)
SELECT 
    query,
    calls,
    total_time,
...
## 💡 Best Practices использования EXPLAIN

### **1. Всегда используйте EXPLAIN ANALYZE для реальных данных**
```sql
-- НЕПРАВИЛЬНО
EXPLAIN SELECT * FROM users;
...
### **2. Анализируйте на реалистичных данных**
```sql
-- Тестируйте на production-like данных
EXPLAIN ANALYZE 
SELECT * FROM orders 
WHERE created_at BETWEEN '2024-01-01' AND '2024-01-31';
```

### **3. Сравнивайте разные варианты запросов**
```sql
-- Вариант A
EXPLAIN ANALYZE 
...
-- Вариант B  
EXPLAIN ANALYZE
SELECT * FROM users WHERE name >= 'John' AND name < 'Johnz';
```

### **4. Мониторьте изменения в планах выполнения**
```sql
-- Регулярно проверяйте критические запросы
EXPLAIN (ANALYZE, BUFFERS) 
...
- ✅ Малое значение cost и actual time
- ✅ Малое количество Rows Removed by Filter
- ✅ Высокий процент Buffers shared hit

### **Красные флаги (проблемы)**
- ❌ Seq Scan на больших таблицах
- ❌ Высокое значение cost и actual time
- ❌ Большое количество Rows Removed by Filter  
- ❌ Nested Loop с большими таблицами
- ❌ Низкий процент Buffers shared hit

### **Желтые флаги (требуют внимания)**
- ⚠️ Bitmap Scan (может быть нормально для сложных условий)
- ⚠️ Hash Join (зависит от размера таблиц)
- ⚠️ Sort operations (проверить необходимость)

EXPLAIN — это мощнейший инструмент для понимания того, как база данных выполняет ваши запросы. Регулярное использование EXPLAIN ANALYZE помогает находить узкие места и значительно улучшать производительность приложений.
# 12. SQL Партиционирование

## 📚 Что такое партиционирование

**Партиционирование** — это метод разделения большой таблицы на меньшие, более управляемые части (партиции) на основе определенных правил. Каждая партиция хранится отдельно, но запросы работают с таблицей как с единым целым.

## 🎯 Зачем нужно партиционирование

### **Преимущества партиционирования:**
- ✅ **Улучшение производительности** запросов (partition pruning)
- ✅ **Упрощение управления** данными (архивация, удаление)
- ✅ **Параллельные операции** с разными партициями
- ✅ **Эффективное хранение** (разные партиции на разных дисках)

### **Когда использовать партиционирование:**
- Таблицы больше 10-100 GB
- Данные имеют естественные границы (даты, регионы)
- Частые операции с подмножествами данных
- Требуется быстрая архивация/удаление старых данных

## 🏗️ Типы партиционирования
...
### **3. Hash Partitioning (Хэш-партиционирование)**
```sql
...
### **Пример 1: Партиционирование логов по месяцам**
```sql
-- Основная таблица
...
-- Автоматическое создание партиций через функцию
...
BEGIN
    ...
    EXECUTE format(
        'CREATE TABLE logs_%s PARTITION OF application_logs FOR VALUES FROM (%L) TO (%L)',
        next_month,
        start_date,
        end_date
    );
...
### **Пример 2: Партиционирование заказов по статусу и дате**
```sql
CREATE TABLE orders (
    order_id BIGSERIAL,
    customer_id INTEGER,
...
-- Создание подпартиционирования по дате для каждого статуса
CREATE TABLE orders_pending PARTITION OF orders
    FOR VALUES IN ('pending') PARTITION BY RANGE (order_date);

CREATE TABLE orders_completed PARTITION OF orders
    FOR VALUES IN ('completed') PARTITION BY RANGE (order_date);

CREATE TABLE orders_cancelled PARTITION OF orders
    FOR VALUES IN ('cancelled') PARTITION BY RANGE (order_date);

-- Подпартиции для completed заказов
CREATE TABLE orders_completed_2023 PARTITION OF orders_completed
    FOR VALUES FROM ('2023-01-01') TO ('2024-01-01');

CREATE TABLE orders_completed_2024 PARTITION OF orders_completed
...
```sql
-- Добавление партиции для нового месяца
CREATE TABLE sales_2024_q1 PARTITION OF sales
    FOR VALUES FROM ('2024-01-01') TO ('2024-04-01');

-- Добавление партиции для новой страны
CREATE TABLE customers_africa PARTITION OF customers
    FOR VALUES IN ('ZA', 'EG', 'NG');
```

### **Удаление партиций**
```sql
-- Удаление старой партиции (данные сохраняются в отдельной таблице)
ALTER TABLE sales DETACH PARTITION sales_2022_q1;

-- Затем можно удалить или архивировать
DROP TABLE sales_2022_q1;

-- Или переименовать для архивации
ALTER TABLE sales_2022_q1 RENAME TO sales_2022_q1_archive;
```

### **Обмен партициями**
```sql
-- Быстрая замена данных в партиции
CREATE TABLE sales_new_data (LIKE sales INCLUDING ALL);

-- Загрузка новых данных
INSERT INTO sales_new_data VALUES (...);

-- Обмен партиции
ALTER TABLE sales DETACH PARTITION sales_current;
ALTER TABLE sales ATTACH PARTITION sales_new_data
    FOR VALUES FROM ('2024-01-01') TO ('2024-04-01');
```
...
```sql
-- Просмотр всех партиций таблицы (PostgreSQL)
SELECT 
...
## ⚡ Оптимизация запросов с партиционированием
...
-- В плане выполнения будет видно:
-- Append
--   -> Seq Scan on sales_2023_q1
--         Filter: (sale_date >= '2023-01-01' AND sale_date <= '2023-01-31')
-- Только партиция sales_2023_q1 будет сканироваться!
```

### **Индексы на партициях**
```sql
-- Глобальный индекс (на всей таблице)
CREATE INDEX idx_sales_date ON sales(sale_date);

-- Локальные индексы (на каждой партиции)
CREATE INDEX idx_sales_2023_q1_date ON sales_2023_q1(sale_date);
CREATE INDEX idx_sales_2023_q1_region ON sales_2023_q1(region);

-- Уникальные индексы должны включать ключ партиционирования
CREATE UNIQUE INDEX idx_sales_id_date ON sales(id, sale_date);
```
...
- ❌ Уникальные индексы должны включать ключ партиционирования
- ❌ Внешние ключи на партиционированные таблицы ограничены
- ❌ Некоторые типы constraints не поддерживаются
- ❌ Сложности с наследованием и триггерами

### **Неподдерживаемые операции:**
```sql
-- Эти операции могут не работать как ожидается:

-- UNIQUE constraint без ключа партиционирования
CREATE TABLE test (id SERIAL, date DATE) PARTITION BY RANGE (date);
-- ❌ CREATE UNIQUE INDEX idx_test_id ON test(id); -- Ошибка!
-- ✅ CREATE UNIQUE INDEX idx_test_id_date ON test(id, date); -- Правильно

-- EXCLUDE constraints
-- ❌ EXCLUDE USING gist (...)
```

## 🔄 Миграция на партиционирование
...
-- 3. Постепенное копирование данных
...
### **Online миграция с минимальным downtime**
```sql
-- Использование логической репликации или триггеров
CREATE OR REPLACE FUNCTION sync_to_partitioned()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO sales_new VALUES (NEW.*);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER sync_sales
...
```sql
-- ХОРОШО: Дата создания, географический регион, категория
CREATE TABLE events (event_time TIMESTAMP, ...) 
PARTITION BY RANGE (event_time);

-- ПЛОХО: ID, случайные значения, часто изменяемые поля
CREATE TABLE users (id SERIAL, ...) 
PARTITION BY HASH (id); -- Обычно не дает преимуществ
```

### **2. Размер партиций**
```sql
-- Оптимальный размер: от 1GB до 10GB
-- Слишком маленькие: много партиций, overhead
-- Слишком большие: теряются преимущества партиционирования

-- Для временных данных: по дням/неделям/месяцам
-- Для справочников: по регионам/категориям
```

### **3. Индексы и constraints**
```sql
-- Всегда создавайте индексы на ключе партиционирования
CREATE INDEX idx_sales_date ON sales(sale_date);

-- Уникальные constraints должны включать ключ партиционирования
CREATE UNIQUE INDEX idx_orders_id_date ON orders(order_id, order_date);
```

### **4. Мониторинг и обслуживание**
```sql
-- Регулярно добавляйте новые партиции
-- Архивируйте/удаляйте старые партиции
-- Мониторьте размеры и производительность
-- Используйте partition-aware backup инструменты
```
...
```sql
-- Горячие партиции на SSD, холодные на HDD
-- Активные партиции реплицируются, архивные - нет
```

### **Партиционирование в облачных базах**
```sql
-- AWS RDS, Google Cloud SQL, Azure Database
-- Автоматическое партиционирование через функции
-- Интеграция с облачным хранилищем для архивных данных
```

### **Партиционирование для шардинга**
```sql
-- Комбинация горизонтального шардинга и вертикального партиционирования
-- Разные партиции на разных серверах
-- Использование FDW (Foreign Data Wrapper) для распределенных запросов
```

## 💡 Заключение

**Партиционирование — это мощный инструмент для:**

- ✅ **Управления большими таблицами** (100GB+)
- ✅ **Ускорения запросов** через partition pruning  
- ✅ **Упрощения операций** с данными (архивация, удаление)
- ✅ **Параллельной обработки** данных

**Ключевые принципы успешного партиционирования:**
- Выбирайте естественные границы партиций
- Следите за размером партиций
- Используйте правильные индексы
- Автоматизируйте управление партициями
- Регулярно мониторьте и оптимизируйте

Партиционирование требует тщательного планирования, но при правильной реализации может значительно улучшить производительность и управляемость больших баз данных.
