...
The final code is about 250 lines of C ([unix](https://www.jmeiners.com/lc3-vm/src/lc3.c), [windows](https://www.jmeiners.com/lc3-vm/src/lc3-win.c)). All you need to know is how to read basic C or C++ and how to do [binary arithmetic](https://www.swarthmore.edu/NatSci/echeeve1/Ref/BinaryMath/BinaryMath.html).

1. [What is a virtual machine?](https://www.jmeiners.com/lc3-vm/#what-is-a-virtual-machine-)
2. [LC-3 architecture](https://www.jmeiners.com/lc3-vm/#lc-3-architecture)
3. [Assembly examples](https://www.jmeiners.com/lc3-vm/#assembly-examples)
4. [Executing programs](https://www.jmeiners.com/lc3-vm/#executing-programs)
5. [Implementing instructions](https://www.jmeiners.com/lc3-vm/#implementing-instructions)
6. [Instruction cheat sheet](https://www.jmeiners.com/lc3-vm/#instruction-cheat-sheet)
7. [Trap routines](https://www.jmeiners.com/lc3-vm/#trap-routines)
8. [Trap routine cheat sheet](https://www.jmeiners.com/lc3-vm/#trap-routine-cheat-sheet)
9. [Loading programs](https://www.jmeiners.com/lc3-vm/#loading-programs)
10. [Memory mapped registers](https://www.jmeiners.com/lc3-vm/#memory-mapped-registers)
11. [Platform specifics](https://www.jmeiners.com/lc3-vm/#platform-specifics)
12. [Running the VM](https://www.jmeiners.com/lc3-vm/#running-the-vm)
13. [Alternate C++ technique](https://www.jmeiners.com/lc3-vm/#alternate-c-technique)
14. [Contributions](https://www.jmeiners.com/lc3-vm/#contributions)

> **Note:** This tutorial is a [literate program](https://en.wikipedia.org/wiki/Literate_programming). This means you are reading the source code right now! Each piece of code from the VM project will be shown and explained thoroughly, so you can be sure nothing is left out. The final code was created by [“tangling”](https://github.com/justinmeiners/srcweave) the blocks of code together.
...
> **Note:** A compiler solves a similar problem by compiling a standard _high-level language_ to several CPU architectures. A VM creates one standard _CPU architecture_ which is simulated on various hardware devices. One advantage of a compiler is that it has no runtime overhead while a VM does. Even though compilers do a pretty good job, writing a new one that targets multiple platforms is very difficult, so VMs are still helpful here. In practice, VMs and compilers are mixed at various levels.
...
**_[Memory Storage](https://www.jmeiners.com/lc3-vm/#:memory-storage)_**
...
Used by [1](https://www.jmeiners.com/lc3-vm/#:lc3.c_2 "/lc3.c") 2 3 4
...
**_[Registers](https://www.jmeiners.com/lc3-vm/#:registers)_**
...
Used by [1](https://www.jmeiners.com/lc3-vm/#:lc3.c "/lc3.c") 2 3 4

Just like the memory, we will store the registers in an array:

**_[Register Storage](https://www.jmeiners.com/lc3-vm/#:register-storage)_**
...
**_[Opcodes](https://www.jmeiners.com/lc3-vm/#:opcodes)_**
...
**_[Condition Flags](https://www.jmeiners.com/lc3-vm/#:condition-flags)_**
...
**_[/lc3.c](https://www.jmeiners.com/lc3-vm/#:lc3.c)_**
...
**_[Hello World Assembly](https://www.jmeiners.com/lc3-vm/#:hello-world-assembly)_**
...
**_[Loop Assembly](https://www.jmeiners.com/lc3-vm/#:loop-assembly)_**
...
3. Look at the _opcode_ to determine which type of instruction it should perform.
...
**_[Main Loop](https://www.jmeiners.com/lc3-vm/#:main-loop)_**
...
Used by [1](https://www.jmeiners.com/lc3-vm/#:lc3.c_2 "/lc3.c") 2
...
**_[Load Arguments](https://www.jmeiners.com/lc3-vm/#:load-arguments)_**
...
Used by [1](https://www.jmeiners.com/lc3-vm/#:main-loop "Main Loop") 2 3
...
So we know where we want to store the result and we know the first number to add. The last bit of information we need is the second number to add. At this point, the two rows start to look different. Notice that on the top row the 5th bit is a `0` and in the second row it is `1`. This bit indicates whether it is _immediate mode_ or _register mode_. In _register mode_, the second number is stored in a register just like the first. This is marked `SR2` and is contained in bits 2-0. Bits 3 and 4 are unused. In assembly this would be written as:

**_[Add Register Assembly](https://www.jmeiners.com/lc3-vm/#:add-register-assembly)_**
...
**_[Add Immediate Assembly](https://www.jmeiners.com/lc3-vm/#:add-immediate-assembly)_**
...
**_[Sign Extend](https://www.jmeiners.com/lc3-vm/#:sign-extend)_**
...
Earlier we defined a condition flags enum, and now it’s time to use them. _Any time_ a value is written to a register, we need to update the flags to indicate its sign. We will write a function so that this can be reused:

**_[Update Flags](https://www.jmeiners.com/lc3-vm/#:update-flags)_**
...
**_[ADD](https://www.jmeiners.com/lc3-vm/#:add)_**
...
Used by [1](https://www.jmeiners.com/lc3-vm/#:main-loop "Main Loop")
...
Just like before, we need to sign extend this 9-bit value, but this time add it to the current `PC`. (If you look back at the execution loop, the `PC` was incremented right after this instruction was loaded.) The resulting sum is an address to a location in memory, and that address _contains, yet another_ value which is the address of the value to load.
...
**_[C LDI Sample](https://www.jmeiners.com/lc3-vm/#:c-ldi-sample)_**
...
**_[LDI](https://www.jmeiners.com/lc3-vm/#:ldi)_**
...
**_[BAD OPCODE](https://www.jmeiners.com/lc3-vm/#:bad-opcode)_**
...
**_[AND](https://www.jmeiners.com/lc3-vm/#:and)_**
...
**_[NOT](https://www.jmeiners.com/lc3-vm/#:not)_**
...
**_[BR](https://www.jmeiners.com/lc3-vm/#:br)_**
...
**_[JMP](https://www.jmeiners.com/lc3-vm/#:jmp)_**
...
**_[JSR](https://www.jmeiners.com/lc3-vm/#:jsr)_**
...
**_[LD](https://www.jmeiners.com/lc3-vm/#:ld)_**
...
**_[LDR](https://www.jmeiners.com/lc3-vm/#:ldr)_**
...
**_[LEA](https://www.jmeiners.com/lc3-vm/#:lea)_**
...
**_[ST](https://www.jmeiners.com/lc3-vm/#:st)_**
...
**_[STI](https://www.jmeiners.com/lc3-vm/#:sti)_**
...
**_[STR](https://www.jmeiners.com/lc3-vm/#:str)_**
...
The LC-3 provides a few predefined routines for performing common tasks and interacting with I/O devices. For example, there are routines for getting input from the keyboard and for displaying strings to the console. These are called _trap routines_ which you can think of as the operating system or API for the LC-3. Each trap routine is assigned a _trap code_ which identifies it (similar to an opcode). To execute one, the `TRAP` instruction is called with the trap code of the desired routine.

![Trap Encoding](https://www.jmeiners.com/lc3-vm/img/trap_layout.gif)

Define an enum for each trap code:

**_[TRAP Codes](https://www.jmeiners.com/lc3-vm/#:trap-codes)_**
...
**_[TRAP](https://www.jmeiners.com/lc3-vm/#:trap)_**
...
Used by [1](https://www.jmeiners.com/lc3-vm/#:main-loop "Main Loop") [2](https://www.jmeiners.com/lc3-vm/#:instruction-c- "Instruction C++")
...
Notice that unlike C strings, characters are _not stored_ in a single byte, but in a _single memory location_. Memory locations in LC-3 are 16 bits, so each character in the string is 16 bits wide. To display this with a C function, we will need to convert each value to a char and output them individually.

**_[TRAP PUTS](https://www.jmeiners.com/lc3-vm/#:trap-puts)_**
...
Used by [1](https://www.jmeiners.com/lc3-vm/#:trap "TRAP")
...
**_[TRAP GETC](https://www.jmeiners.com/lc3-vm/#:trap-getc)_**
...
**_[TRAP OUT](https://www.jmeiners.com/lc3-vm/#:trap-out)_**
...
**_[TRAP IN](https://www.jmeiners.com/lc3-vm/#:trap-in)_**
...
**_[TRAP PUTSP](https://www.jmeiners.com/lc3-vm/#:trap-putsp)_**
...
**_[TRAP HALT](https://www.jmeiners.com/lc3-vm/#:trap-halt)_**
...
**_[Read Image File](https://www.jmeiners.com/lc3-vm/#:read-image-file)_**
...
**_[Swap](https://www.jmeiners.com/lc3-vm/#:swap)_**
...
> **Note:** [Endianness](https://en.wikipedia.org/wiki/Endianness) refers to how bytes of an integer are interpreted. In little-endian, the first byte is the least significant digit, and in big-endian, it is reversed. As far as I know, the decision is _mostly_ arbitrary. Different companies made different decisions, so now we are left with varying implementations. You do not need to know anything else about endianness for this project.

Let’s also add a convenience function for `read_image_file` which takes a path a string;

**_[Read Image](https://www.jmeiners.com/lc3-vm/#:read-image)_**
...
**_[Memory Mapped Registers](https://www.jmeiners.com/lc3-vm/#:memory-mapped-registers)_**
...
**_[Memory Access](https://www.jmeiners.com/lc3-vm/#:memory-access)_**
...
**_[Input Buffering](https://www.jmeiners.com/lc3-vm/#:input-buffering)_**
...
**_[Includes](https://www.jmeiners.com/lc3-vm/#:includes)_**
...
Used by [1](https://www.jmeiners.com/lc3-vm/#:lc3.c "/lc3.c") 2

### Windows

NOTE: If you already included the **Unix** versions don’t add these!

**_[Input Buffering Windows](https://www.jmeiners.com/lc3-vm/#:input-buffering-windows)_**
...
**_[Windows Includes](https://www.jmeiners.com/lc3-vm/#:windows-includes)_**
...
**_[Setup](https://www.jmeiners.com/lc3-vm/#:setup)_**
...
**_[Shutdown](https://www.jmeiners.com/lc3-vm/#:shutdown)_**
...
**_[Handle Interrupt](https://www.jmeiners.com/lc3-vm/#:handle-interrupt)_**
...
**_[/lc3.c](https://www.jmeiners.com/lc3-vm/#:lc3.c_2)_** [+=](https://www.jmeiners.com/lc3-vm/#:lc3.c)
...
**_[Instruction C++](https://www.jmeiners.com/lc3-vm/#:instruction-c-)_**
...
**_[Op Table](https://www.jmeiners.com/lc3-vm/#:op-table)_**
...
> **Note:** I learned about this technique from [Bisqwit’s NES emulator](https://www.youtube.com/watch?v=QIUVSD3yqqE). If you are interested in emulation, or the NES, I highly recommend his videos.

The rest of the C++ version uses the code we already wrote! The full source is here: [unix](https://www.jmeiners.com/lc3-vm/src/lc3-alt.cpp), [windows](https://www.jmeiners.com/lc3-vm/src/lc3-alt-win.cpp).

## Contributions

[atul-g](https://github.com/atul-g/rust_vm) has contributed a handy [reference card](https://www.jmeiners.com/lc3-vm/supplies/lc3-ref-card.png) that summarizes how the whole system works.
...
Want to contribute? We need help with an integration test. This is a good [first issue](https://github.com/justinmeiners/lc3-vm/issues/13) to learn from.