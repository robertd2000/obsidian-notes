...
function reduceTree<T extends object, S>(
    ...
    initialState: S,
    traversal: 'pre-order' | 'post-order' = 'pre-order'
): S {
    function traverse(node: TreeItem<T>, currentState: S): S {
        let newState = currentState;

        // Pre-order: обрабатываем текущий узел до детей
        if (traversal === 'pre-order') {
            newState = reducer(newState, node);
        }

        // Рекурсивно обрабатываем детей
        for (const child of node.children) {
            newState = traverse(child, newState);
        }

        // Post-order: обрабатываем текущий узел после детей
    ...
    return traverse(root, initialState);
}

// Альтернативная итеративная реализация (избегает рекурсии для больших деревьев):
function reduceTreeIterative<T extends object, S>(
    root: TreeItem<T>,
    reducer: (state: S, item: T) => S,
    initialState: S,
    traversal: 'pre-order' | 'post-order' = 'pre-order'
    ...
    const stack: { node: TreeItem<T>; visited: boolean }[] = [];
    ...
    while (stack.length > 0) {
        ...
        if (current.visited) {
            ...
            state = reducer(state, current.node);
            continue;
        }

        if (traversal === 'post-order') {
            // Для post-order помечаем узел как посещенный и кладем обратно
...
type CategoryTree = TreeItem<Category>;
...
// Pre-order: сумма значений (Root → Child 1 → Grandchild 1 → Child 2)
const preOrderSum = reduceTree(
    categoryTree,
    (sum, item) => sum + item.value,
    0,
    'pre-order'
);
console.log('Pre-order sum:', preOrderSum); // 10 + 5 + 2 + 8 = 25

// Post-order: сумма значений (Grandchild 1 → Child 1 → Child 2 → Root)
const postOrderSum = reduceTree(
    categoryTree,
    (sum, item) => sum + item.value,
    0,
    'post-order'
);
console.log('Post-order sum:', postOrderSum); // 2 + 5 + 8 + 10 = 25
...
console.log('All IDs:', allIds); // [1, 2, 3, 4]

// Поиск максимального значения
const maxValue = reduceTree(
    categoryTree,
